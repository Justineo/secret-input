...
Native password inputs provide a reference for interaction and privacy behavior. Acceptance depends on completing these tasks, without requiring identical password-field behavior on every platform. Continue using native password inputs for login passwords.
...
| Avoid incorrect fills      | Unexpected browser/extension DOM writes cannot change the authoritative value or submission; unrelated suggestions are reduced where possible                                        | Value isolation is a core guarantee; suppressing third-party suggestions is best effort                                                              |
...
If undo/redo later becomes a product guarantee across devices, expose public history operations and queryable availability so applications can provide buttons accessible by touch, keyboard, and screen reader. Do not simulate native browser history through temporary plaintext editing or deprecated commands. This is a future API direction, not an existing public capability.
...
For non-login configuration involving API keys, tokens, and signing keys, retain the current approach: one authoritative value, a native text-input surface, and bullet presentation. The benefits of value isolation justify the editing controller's cost. Do not expand it into a general editor to match every native password feature.
